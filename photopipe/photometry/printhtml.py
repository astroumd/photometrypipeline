...
OUTPUT:
    photcomp.plot.png - shows magnitude vs. error for each band_filter
    abmag-energy.plot.png - shows flux vs. magnitude for each band_filter
...
def plot_mag(filters, plotdict, colors):
    ...
    # For each band_filter, plot mag vs. error for photocomp.png
    counter = 0
    for band_filter in filters:
        pl.plot(
            plotdict[band_filter+'mag'], plotdict[band_filter+'magerr'], marker='o', linestyle='None',
            label=band_filter, color=colors[counter]
        )
        counter = counter + 1
...
def plot_seds(filters):
    wavelengths = {
        'r': 6122.3, 'i': 7439.5, 'z': 8897.1, 'y': 10289.4, 'J': 12350.0, 'K': 21590.0, 'u': 3594.9, 'g': 4640.4,
        'H': 16620.0
    }
    file_mask = [
        'RA', 'DEC', 'u', 'g', 'r', 'i', 'z', 'y', 'B', 'V', 'R', 'I', 'J', 'H', 'K', 'u_err', 'g_err', 'r_err',
        'i_err', 'z_err', 'y_err', 'B_err', 'V_err', 'R_err', 'I_err', 'J_err', 'H_err', 'K_err', 'Mode', 'Timestamp',
        'Filter'
    ]
    file_formats = (
        '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8',
        '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', 'U18', 'U1'
    )
    ...
    for file in files:
        ...
        for i in range(data.size):
            band_filter = (data[i])[file_mask.index('Filter')]
            filters_map[band_filter] = np.array([data[i]]) \
                if not (band_filter in filters_map) else np.append(filters_map[band_filter], np.array([data[i]]), 0)

        ncols = (2 if len(filters_map) > 1 else 1)
        nrows = (int(np.ceil(len(filters_map) / float(2))) if len(filters_map) > 1 else 1)
        ...
        pl.subplots_adjust(hspace=0.9, wspace=0.3)

        ax_row = 0
        ax_col = 0 if len(filters_map) > 2 else 1

        for band_filter, seds in filters_map.items():
            ...
            col.set_title('Filter ' + band_filter)
            col.set_xlabel(r'Wavelength ($\AA$)')
            col.set_ylabel('Magnitude')
            
            plot_label = datetime.strptime(
                sed[file_mask.index('Timestamp')], '%Y%m%dT%H%M%S%f'
            ...
            col.legend(
                loc='upper center', bbox_to_anchor=(0.5, -0.4), fancybox=True, shadow=False, ncol=5, prop={'size': 11}
            )
...
def printhtml(filters, colnames, omitted_colnames, headers, headers_names):
    ...
    # Finds band_filter image files with green circles over sources and writes to HTML
    prefchar = 'coadd'
    ...
    f.write(
        'Notes: Non-zero magnitudes with uncertainty of zero are 3-sigma upper limits.  ' +
        'Sources with magnitudes of 0.0000 are unobserved.<BR>\n'
    )

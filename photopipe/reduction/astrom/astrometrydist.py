...
def distance(obj1, obj2):
    ...
    # Calculates the great circle distance using the Haversine formula which
    # is best suited for small angles (i.e. small separation)
    dist_rad = 2 * numpy.arcsin(
        numpy.sqrt((numpy.sin(ddec/2.))**2 + numpy.cos(obj1.dec_rad) * numpy.cos(obj2.dec_rad) * (numpy.sin(dra/2.))**2)
    )
...
def tooclose(glist, minsep=3, quiet=False):
    ...
    INPUTS:
        ...
        minsep - minimum separation allowed, will remove objects closer than this value.
            Set in arcsecs and default set to 3
    OUTPUTS:
...
def quickdistance(obj1, obj2, cosdec):
    ...
    INPUTS:
        obj1, obj2 - Must be objects created from the Obj class specified in astrometrysources.py
        cosdec - the cos(declination) that you want to calculate the distance at.  This is usually a mean or median of
            a set
    OUTPUTS:
...
def posangle(obj1, obj2):
    ...
    pa_rad = numpy.arctan2(
        numpy.cos(obj1.dec_rad)*numpy.tan(obj2.dec_rad)-numpy.sin(obj1.dec_rad)*numpy.cos(dra), numpy.sin(dra)
    )
...
def calcdist(slist, maxrad, minrad, rascale):
    ...
    INPUTS:
        ...
        rascale - the cos(declination) that you want to calculate the distance at.  This is usually a mean or median of
            a set
    OUTPUTS:
...
def distmatch(
    sexlist, catlist, maxrad=180, minrad=10, reqmatch=3, patolerance=1.2, uncpa=-1, showmatches=0, fastmatch=1,
    quiet=False
):
    """
    NAME:
    ...
    INPUTS:
        ...
        fastmatch   - keyword that exits the for loop once 16 great matches are found (saves computing time for dense
            fields)
    OUTPUTS:
        Returns lists of sextractor sources indices that match catalog source indices and the position angle between the
            two
    EXAMPLE:
    ...
    for si in range(len(sexdists)):
        ...
        for ci in range(len(catdists)):
            ...
            # Find difference in position angle between matched angle between sextractor
            # source A and matched source with catalog source B and matched catalog source.
            ...
            if match >= reqmatch:
                ...
                ndegeneracies = len(smatchin)-len(astrometrystats.unique(smatchin)) + len(cmatchin) - \
                    len(astrometrystats.unique(cmatchin))
    ...
    # Returns sextractor sources and catalog sources that appear to have matches
    # along with the mode of the position angle

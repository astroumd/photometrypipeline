...
def autoastrometry(
    ...
    if pixelscale > 0 and pa > -360:
        ...
        if abs(equinox - 2000) > 0.5:
            ...
            try:
                j2000 = ephem.Equatorial(ephem.Equatorial(str(ra / 15), str(dec), epoch=str(equinox)),
                                         epoch=ephem.J2000)
    ...
    centerra = cra - centerdx * xscale * numpy.cos(initpa * numpy.pi / 180.) + centerdy * yscale * numpy.sin(
        initpa * numpy.pi / 180.)
    centerdec = cdec + parity * centerdx * xscale * numpy.sin(-initpa * numpy.pi / 180.) + \
        centerdy * yscale * numpy.cos(initpa * numpy.pi / 180.)
    # this has only been checked for a PA of zero.

    if not quiet:
        print('cra=%10.6f, centerdx=%10.6f, xscale=%10.6f, centerdy=%10.6f, yscale=%10.6f' % (
            cra, centerdx, xscale, centerdy, yscale
        ))
    ...
    goodsexlist = astrometrysources.sextract(
        sfilename, nxpix, nypix, 3, 12, minfwhm=minfwhm, maxfwhm=maxfwhm, maxellip=maxellip, saturation=saturation,
        sexpath=sexpath, quiet=quiet
    )
    ...
    # If no catalog specified, find catalog with > 15 entries for center
    # RA and DEC encircled by radius of 180 arcseconds
    ...
    minrad = 5.0
    if maxrad == -1:
        # 60 arcsec/arcmin *sqrt(15 stars/(stars/arcsec^2)) / 2 <-- density is for a box
        # so half the length of the box is the radius
        maxrad = 30.0 * (15.0 / min(density, catdensity)) ** 0.5
        maxrad = max(maxrad, 60.0)

        if maxrad == 60.0:
            minrad = 10.0
            # in theory could scale this up further to reduce #comparisons i.e. instead of 15 stars, x stars
    ...
    # in between min and max radius). NOTE: density is per arcmin^2,
    # while the radii are in arcsec, hence the conversion factor.
    ...
    expectfalsetrios = ngood * ncat * circdensity ** 2 * circcatdensity ** 2 * tolerance ** 2 * (
                patolerance / 360.) ** 1
    ...
    # Default required match is 3 (triangle), but can require more if we expect a log of false triples or less
    # if not many sources in either image
    reqmatch = 3
    ...
    (primarymatchs, primarymatchc, mpa) = astrometrydist.distmatch(
        goodsexlist, catlist, maxrad, minrad, reqmatch, patolerance, uncpa, showmatches=showmatches,
        fastmatch=fastmatch, quiet=quiet
    )
    ...
    if outfile == '':
        ...
        directory = filename[0:slashpos + 1]
        fil = filename[slashpos + 1:]
        outfile = directory + 'a' + fil  # alternate behavior would always output to current directory
    try:
        os.remove(outfile)
    except OSError:
        pass
...
def main():
    ...
    parser.add_option(
        "-l", "--sat", help="saturation level (do not use stars exceeding)", default=-1, action="store",
        type="float", dest="sat"
    )

    parser.add_option(
        "-x", "--pix", help="pixel scale in arcsec/pixel (within 1%)", default=-1, action="store",
        type="float", dest="pix"
    )

    parser.add_option(
        "-b", "--box", help="half-width of box for reference catalog query (arcsec)", default=-1, action="store",
        type="float", dest="box"
    )

    parser.add_option(
        "-s", "--see", help="approximate seeing in pixels for CR/star/galaxy ID'ing", default=-1, action="store",
        type="float", dest="see"
    )

    parser.add_option(
        "-u", "--upa", help="Uncertainty of the position angle (degrees)", default=-1, action="store",
        type="float", dest="upa"
    )

    parser.add_option(
        "-c", "--catalog", help="Catalog to use (tmpsc, ub2, tmc, sdss, or file: see --catalog)", default='',
        action="store", dest="cat"
    )

    parser.add_option(
        "-t", "--toler", help="Amount of slack allowed in match determination", default=defaulttolerance,
        action="store", type="float", dest="tol"
    )

    parser.add_option(
        "-e", help="Maximum ellipticity", default=-1, action="store", type="float", dest="mel"
    )

    parser.add_option(
        "-p", "--pa", help="The position angle in degrees.  Not usually needed.", default=-999, action="store",
        type="float", dest="pa"
    )

    parser.add_option(
        "-i", "--inv", help="Reverse(=positive) parity.", default=False, action="store_true", dest="inv"
    )

    parser.add_option(
        "-q", "--quiet", help="Quiet", default=False, action="store_true", dest="quiet"
    )

    parser.add_option(
        "-m", help="Maximum distance to look for star pairs (in radius).", default=-1, action="store",
        type="float", dest="maxrad"
    )

    parser.add_option(
        "-r", "--ra", help="Right ascension", default='', action="store", dest="userra"
    )

    parser.add_option(
        "-d", "--dec", help="Declination", default='', action="store", dest="userdec"
    )

    parser.add_option(
        "-o", "--output", help="Specify output file (no argument overwrites input file)", default='', action="store",
        dest="outfile"
    )

    parser.add_option(
        "-n", "--nosolve", help="Do not attempt to solve astrometry; just write catalog.", default='', action="store",
        dest="nosolvecat"
    )
    ...
    # Turns userra and userdec into degrees or sets number default
    # (parser stored as strings because it is in sexagesimal)
    if options.userra == '':
        userra = -999
    else:
    ...
    for f in files:
        ...
        fitinfo = autoastrometry(
            f, pixelscale=options.pix, pa=options.pa, inv=inv,

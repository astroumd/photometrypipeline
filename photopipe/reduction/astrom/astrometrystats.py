...
def most(pylist, vmin=1, vmax=1):
    ...
    EXAMPLE:
        most(numpy.array([1.2, 4.3, 3.1, 5.1, 3.9, 7.1]), vmin=1, vmax=1)
            ---> 4.3
    """
    counter = numpy.zeros(len(pylist))

    # Finds number of elements that are between current element +/- vmax/vmin
    for i in range(0, len(pylist)):
        counter[i] = ((pylist[i] + vmax >= pylist) & (pylist[i] - vmin <= pylist)).sum()
    ...
    if len(set(counter)) == 1:
        return numpy.median(pylist)
    else:
        return pylist[counter.argmax()]

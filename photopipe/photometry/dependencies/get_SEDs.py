...
of online catalogs (USNOB1, 2MASS, SDSS, PANSTARRS) and synthetic photometry.
...
FILTER_PARAMS = {
    'u': (3551., 8.6387e-9, 558.4, 0), 'g': (4686., 4.9607e-9, 1158.4, 1),
...
class online_catalog_query:
    ...
    Standard usage:
     ...
     If an ignore argument (string or list: ['sdss', 'panstarrs','apass','usnob']) is included,
    ...
    def _parse_panstarrs(s):
        ...
        s: a string as returned by Vizier using Catologue ID 'II/349'
        returns: a 2-d array, with each row containing the results for one object:
          [ra, dec, g, g_sig, r, r_sig, i, i_sig, z, z_sig, y, y_sig]
        """
    ...
    def _query_panstarrs(self, container=None, cont_index=4):
        """
            Query Vizier server for sources found in a box of width self.boxsize (arcmins)
            around self.coords.
            Returns an array (if objects found) or None (if not)
        ...
        # search for PANSTARRS objects around coordinates with
        #  defined box size, return only basic parameters, and
        #  sort by distance from coordinates, and return only those
        #  sources brighter than trim_mag
    ...
    def _parse_2mass(s):
        ...
        for line in lines:
            try:
                ...
                tmp = [ra, dec]
                for bnd in line[2:5]:
                    mag, err = [val for val in bnd.split(' ') if val]
                    mag = float(mag)
                    if '-' not in err:
...
def find_field(coords, extend=3.):
    ...
    X_ra = lambda r, d: (np.cos(d) * np.sin(r - r_c)) / (
            np.cos(d_c) * np.cos(d) * np.cos(r - r_c) + np.sin(d_c) * np.sin(d))
...
def fit_sources(inn, f_err=None, return_cut=False):
    """
    ...
    #             0, 0, 0, 0, 1, 1, 1]
        ...
        raise ValueError("Mode can only be one of 0, 1, 2, 3")
    ...
    for np_band in npALL_FILTERS[~mask]:
        i_band = FILTER_PARAMS[np_band][-1]
        # temporary fill-in for when USNOB bands are gone or get cut
        if (mode_inn == 2) and (np_band in ['B', 'R']):  # note: assuming we always model I
            full_errs[i_band] = 0.5
        # temporary fill-in for when APASS bands are gone or get cut
        elif (mode_inn == 1) and (np_band in ['B', 'V', 'g', 'r', 'i']):
            full_errs[i_band] = 0.25
        else:
            full_errs[i_band] = f_err[mode_inn][np_band](min(err, f_err[mode_inn]['range'][1]))

    # if a value was cut while fitting, return the modeled magnitude instead of the observed
    if return_cut and i_cut is not None:
...
class catalog:
    ...
    The main function produces, for any requested field,
     ...
     2-MASS and (SDSS, PanSTARRs, or USNOB1), with any missing
    ...
    Optional arguments:
     ...
     ignore: option to ignore any of the bands but 2mass. Can be list or single item, options
      are:  ["sdss", "panstarrs", "usnob", "apass"]
    """
    MAX_SIZE = 7200  # max size of largest single query
    ERR_CUT = (8., 5., 2.5)  # maximum reduced chi^2 to keep a fit (SDSS, PANSTARRS, APASS, USNOB)
    ...
    def __init__(self, field_center, field_width, input_coords=None, ignore=None):
        ...
        #  can be list or string in ['usnob','apass','sdss', 'panstarrs']
        self.ignore = ignore
    ...
    def produce_catalog(self):
        ...
        if mass is not None:
            ...
            #  of all objects present in 2mass and (sdss or panstarrs or apass or usnob)
            #  Preference ranking: 2MASS + (SDSS > PanSTARRs > APASS > USNOB)
        ...
        for i, irow in enumerate(results):
            # each row is (sed, full_errs, model_err, index)
            if irow[2] > self.ERR_CUT[modes[i]]:  # apply quality-of-fit cut
                self.numcut += 1
                pass
            else:
                self.coords.append(object_coords[i])
                self.SEDs.append(irow[0])
                self.full_errors.append(irow[1])
                self.model_errors.append(irow[2])
                self.models.append(irow[3])
    ...
    def get_source(self, ra, dec):
        ...
        if index < 0:
            ...
            for i, iband in enumerate(self.bands):
                out_dict[iband] = [self.SEDs[index][i], self.full_errors[index][i]]
            return out_dict
...
def save_catalog(coordinates, seds, errors, modes, file_name):
    ...
    fff.write('# Observed/modeled SEDs produced by get_SEDs.py \n' +
              ...
              '#       = 1: -> B,V,R,I,g modeled from PanSTARRs and 2-MASS\n' +
              '#       = 2: -> u,y,R,I modeled from APASS and 2-MASS\n' +
              '#       = 3: -> u,g,r,i,z,y,V,I modeled from USNOB-1 and 2-MASS\n' +
              "# " + "RA".ljust(10) + "DEC".ljust(12) + "".join([f.ljust(8) for f in ALL_FILTERS]) +
              "".join([(f + "_err").ljust(8) for f in ALL_FILTERS]) + "Mode\n")
    for i, irow in enumerate(seds):
        row_txt = "".join([s.ljust(12) for s in map(lambda x: "%.6f" % x, coordinates[i])]) + \
                  "".join([s.ljust(8) for s in map(lambda x: "%.3f" % x, irow)]) + \
...
def calc_zeropoint(input_coords, catalog_coords, input_mags, catalog_mags, clip=True, return_zps=True):
    ...
    Returns: zeropoint (mags), the median average deviation, and a list of matched
        indices for input and catalog sources.
    """

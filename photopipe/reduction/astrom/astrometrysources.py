...
def sextract(sexfilename, nxpix, nypix, border=3, corner=12, minfwhm=1.5, maxfwhm=25, maxellip=0.5, saturation=-1,
    ...
    EXAMPLE:
        goodsexlist = sextract(
            slist, 1024, 1024, border=3, corner=12, minfwhm=1.5, maxfwhm=25, maxellip=0.5, saturation=55000, sexpath=''
        )
    """
    if maxellip == -1:
        maxellip = 0.5
    ...
    for value in sortedgoodsext:
        ...
        ind_obj = SexObj(value)
        goodsexlist.append(ind_obj)
...
def getcatalog(catalog, ra, dec, boxsize, rawidth, decwidth, minmag=8.0, maxmag=-1, maxpm=60.):
    ...
    NAME:
        getcatalog
    PURPOSE:
        Gets catalog given the center RA, DEC, and box size.  If given a standard catalog, grabs from harvard page,
        otherwise will attempt to open given file.  Saves RA, DEC, mag (in that column order) to an output array for
        values within magnitute
    ...
    # Finds which lines are not comments (anything before a line starting with '-----' only for
    #     regular catalogs (user catalog has no comments)
